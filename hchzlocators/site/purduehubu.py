# load default page objects, widgets, and locators
from hubcheck.pageobjects.m_osr_1_3_0 import *


# load page objects overrides


# load widgets overrides
from hubcheck.pageobjects.widgets.header import Header2 as Header


# load page object locator overrides


# load widget locator overrides
from hubcheck.pageobjects.widgets.header import Header2_Locators_Base as Header_Locators
